...
fn player_action(
    ...
    mut target_query: Query<&mut ActionReceivedEventQueue>,
    player_queue: Query<&Target, With<PlayerMarker>>,
) {
    for action in actions.iter() {
        if action.just_pressed(ActionBarAction::Button1) {
            let player_target = player_queue
                .get_single()
                .expect("Cannot find player")
                .targeted_entity;
            if let Some(entity) = player_target {
                let mut target_event_queue =
                    target_query.get_mut(entity).expect("Target does not exist");
                target_event_queue.events.push(ActionReceivedEvent {
